...
def JoinServerReact(
    ...
    for token in user_accounts:
        ...
        if joined:
            verification = da.react_to_verify(channel_id, message_id, react_btn)
            if verification:
                verified_accounts += 1
                print(
                    Fore.GREEN
                    + "[{}] Successfully Verified On Server | Verified = {}".format(
                        str(datetime.now()), verified_accounts
                    )
                )
                write_to_file("verified", name, token)
...
def JoinServerReactConfirmation(
    token,
    user_accounts,
    server_id,
    server_invite_id,
    ...
    for token in user_accounts:
        ...
        if joined:
            member_verified = da.post_member_verification(server_id, proxy)
            if member_verified:
                print(Fore.GREEN + "[{}] Accepted T&C".format(str(datetime.now())))
                verification = da.react_to_verify(channel_id, message_id, react_btn)
                if verification:
                    verified_accounts += 1
                    print(
                ...
                print(
                    Fore.RED + "[{}] Unable To Accept T&C".format(str(datetime.now()))
                )
                write_to_file("unverified", name, token)
        else:
            print(Fore.RED + "[{}] Unable To Join Server".format(str(datetime.now())))
